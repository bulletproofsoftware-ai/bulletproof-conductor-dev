...
def emit_http(record, sink_config):
    ...
    Target URL is operator-configured in conductor-state.json.audit_sink.syslog_target.
    Scheme is allowlisted to http(s) only — rejects file://, ftp://, gopher://, etc. that
    would otherwise let urllib.urlopen exfiltrate the audit payload or read local files.
    """
    target = sink_config.get("syslog_target")
    if not target:
        raise ValueError("http transport requires syslog_target as URL")

    # Allowlist scheme before urlopen (SSRF/local-file mitigation)
    from urllib.parse import urlparse
    parsed = urlparse(target)
    if parsed.scheme not in _ALLOWED_HTTP_SCHEMES:
        raise ValueError(
            f"http transport rejected target scheme '{parsed.scheme}' — allowed: {sorted(_ALLOWED_HTTP_SCHEMES)}"
        )
    ...
    req = urllib.request.Request(target, data=payload, headers=headers, method="POST")  # noqa: S310 -- scheme allowlisted above
    with urllib.request.urlopen(req, timeout=5) as resp:  # noqa: S310 -- scheme allowlisted above
        if resp.status >= 400:
